...
/*
    Copyright (C) 2004, Juan Pedro Bolivar Puente

    SDL_2DGL is free software; you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation; either version 2 of the License, or
    (at your option) any later version.

    SDL_2DGL is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with SDL_2DGL; if not, write to the Free Software
...
void GL2D_InitScreenGL (SDL_Surface *scr, int w, int h, int bpp, Uint8 use_fullscreen)
{
    /* Sets GL attributes */
...
void GL2D_BlitSurfaceGL (GL2D_SurfaceGL * src, SDL_Rect * src_r, SDL_Rect * dest_r, Uint8 alpha)
{
	float x, y;
    ...
    if (src_r == NULL)
		...
		GL2D_BlitGLpart(src, (float)src_r->x, (float)src_r->y, (float)x, (float)y, 
			(float)src_r->w, (float)src_r->h, alpha);
}
...
void GL2D_BlitGLpart(GL2D_SurfaceGL * src, 
			float sx, float sy, float x, float y, float w, float h, Uint8 alpha)
{
    float pw = power_of_two((src->w));
    float ph = power_of_two((src->h));
...
void GL2D_BlitGLrot(GL2D_SurfaceGL * src, float x, float y, float angle, Uint8 alpha)
{
...
void GL2D_CreateGL(SDL_Surface * surf, GLuint * tex, GLint filter)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, 4, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, conv->pixels);
...
static SDL_Surface * CropSurface(SDL_Surface * surf, SDL_Rect * rect)
{
    Uint32 saved_flags;
    Uint8  saved_alpha;
    Uint32 w, h;
...
GL2D_LargeSurfaceGL * GL2D_CreateLargeSurfaceGL(SDL_Surface * surf,GLint filter)
{
...
void GL2D_BlitLargeGL(GL2D_LargeSurfaceGL * src, float x, float y, Uint8 alpha)
{
	int i,j;
	
	for (j = 0; j < src->vParts; j++) {
		for(i = 0; i < src->hParts; i++) {
			GL2D_BlitGL(src->pics[i][j], x+i*src->partSize, y+j*src->partSize, alpha);
		}
	}
}
			...
			float sx, float sy, float x, float y, float w, float h, Uint8 alpha)
{
	...
	for(j = j0; j <= jn; j++)
	{
		if (j == j0) sy2 = sy - j * src->partSize;
		else sy2 = 0;
		
		if (j == jn) {
			if (j == j0) h2 = h;
			else h2 = (sy + h) - (j * src->partSize);
		} else {
			h2 = src->partSize - sy2;
        }
		...
		for(i = i0; i <= in; i++)
		{
			...
			if (i == i0) sx2 = sx - (i * src->partSize);
			else sx2 = 0;
					
			if (i == in) {
				if (i == i0) w2 = w;
				else w2 = (sx + w) - (i*src->partSize);
			}
			else {
				w2 = src->partSize - sx2;
            }		    		
			GL2D_BlitGLpart(src->pics[i][j], sx2, sy2, x+xCount, y+yCount, w2, h2, alpha);
			xCount += w2;			
		}
		yCount += h2;
	}
...
void GL2D_BlitLargeSurfaceGL (GL2D_LargeSurfaceGL * src, SDL_Rect * src_r, SDL_Rect * dest_r, Uint8 alpha)
{
	float x, y;
	...
	if (src_r == NULL)
		...
		GL2D_BlitLargeGLpart(src, (float)src_r->x, (float)src_r->y, (float)x, (float)y, 
			(float)src_r->w, (float)src_r->h, alpha);
